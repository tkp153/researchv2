...
class ApproachNavi(Node):
    def __init__(self):
        ...
        self.accept = True
        self.count = 0
        self.filename = "waypoint.csv"
        
    def callback(self,waypoints):

        if(self.count == 0 and self.accept == True):
        ...
        if(self.accept == True and self.count > 1): 
            time.sleep(1)
            self.accept = False
            pos_x = waypoints.pose.position.x 
            pos_y = waypoints.pose.position.y
    ...
    def set_waypoint(self):
        ...
        if self.goal_handle.accepted:
            self.get_logger().info("Ready for accepting")
            self.accept = True
            self.result_future = self.goal_handle.get_result_async()
            self.result_future.add_done_callback(self.get_result_callback)
            
        elif not self.goal_handle.accepted:
